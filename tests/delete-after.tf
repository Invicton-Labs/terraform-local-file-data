...
module "check_delete_after_exists" {
  ...
  error_message = "delete-after (exists): expected file to exist, but it does not; num chunks: ${module.delete_after.num_chunks}, created: ${module.delete_after.created}, filename: ${module.delete_after.filename}, datasource: ${jsonencode(module.delete_after.datasource)}, files: ${jsonencode(fileset(module.delete_after.created ? "./../" : null, "**"))}"
}
